...
const options_schema_js_1 = require("./options-schema.js");
...
class WebpackAssetsManifest {
    ...
    #isMerging = false;
    hooks = Object.freeze({
        apply: new tapable_1.SyncHook(['manifest']),
        customize: new tapable_1.SyncWaterfallHook(['entry', 'original', 'manifest', 'asset']),
        transform: new tapable_1.SyncWaterfallHook(['assets', 'manifest']),
        done: new tapable_1.AsyncSeriesHook(['manifest', 'stats']),
        options: new tapable_1.SyncWaterfallHook(['options']),
        afterOptions: new tapable_1.SyncHook(['options', 'manifest']),
    });
    constructor(options = {}) {
        this.hooks.transform.tap(PLUGIN_NAME, (assets) => {
            const { sortManifest } = this.options;
            return sortManifest
                ? (0, helpers_js_1.getSortedObject)(assets, typeof sortManifest === 'function' ? sortManifest.bind(this) : undefined)
                : assets;
        });
        this.hooks.afterOptions.tap(PLUGIN_NAME, (options, manifest) => {
            manifest.options = Object.assign(manifest.defaultOptions, options);
            (0, schema_utils_1.validate)(options_schema_js_1.optionsSchema, manifest.options, { name: PLUGIN_NAME });
            manifest.options.output = (0, node_path_1.normalize)(manifest.options.output);
    ...
    get utils() {
        return {
            isKeyValuePair: type_predicate_js_1.isKeyValuePair,
            isObject: type_predicate_js_1.isObject,
            getSRIHash: helpers_js_1.getSRIHash,
        };
    }
    get defaultOptions() {
        return {
    ...
    getExtension(filename) {
        ...
        return (0, node_path_1.extname)(filename);
    }
    fixKey(key) {
    ...
    set(key, value) {
        ...
        if ((0, type_predicate_js_1.isKeyValuePair)(entry)) {
    ...
    processAssetsByChunkName(assets, hmrFiles) {
        if (assets) {
            Object.keys(assets).forEach((chunkName) => {
                (0, helpers_js_1.asArray)(assets[chunkName])
    ...
    async maybeMerge() {
        if (this.options.merge) {
            try {
                const deepmerge = (await Promise.resolve().then(() => __importStar(require('deepmerge')))).default;
                this.#isMerging = true;
                const content = await (0, promises_1.readFile)(this.getOutputPath(), { encoding: 'utf8' });
                const data = JSON.parse(content);
                const arrayMerge = (_destArray, srcArray) => srcArray;
                for (const [key, oldValue] of Object.entries(data)) {
                    if (this.has(key)) {
                        const currentValue = this.get(key);
                        if ((0, type_predicate_js_1.isObject)(oldValue) && (0, type_predicate_js_1.isObject)(currentValue)) {
        ...
        assets?.forEach((asset) => {
            if (asset.name && asset.info.sourceFilename) {
                this.assetNames.set(contextRelativeKeys
                    ...
                    : (0, node_path_1.join)((0, node_path_1.dirname)(asset.name), (0, node_path_1.basename)(asset.info.sourceFilename)), asset.name);
            }
        });
    }
    ...
    async handleProcessAssetsReport(compilation) {
        ...
        const findAssetKeys = (0, helpers_js_1.findMapKeysByValue)(this.assetNames);
        const { contextRelativeKeys } = this.options;
        for (const asset of assets) {
            const sourceFilenames = findAssetKeys(asset.name);
            if (!sourceFilenames.length) {
                const { sourceFilename } = asset.info;
                const name = sourceFilename ? (contextRelativeKeys ? sourceFilename : (0, node_path_1.basename)(sourceFilename)) : asset.name;
        ...
        if (this.options.entrypoints) {
            ...
            const entrypoints = Object.fromEntries(Array.from(compilation.entrypoints, ([name, entrypoint]) => {
                const value = {
                    assets: (0, helpers_js_1.group)(entrypoint.getFiles().filter(removeHMR), getExtensionGroup, getAssetOrFilename),
                };
                const childAssets = stats.namedChunkGroups?.[name]?.childAssets;
                if (childAssets) {
                    for (const [property, assets] of Object.entries(childAssets)) {
                        value[property] = (0, helpers_js_1.group)(assets.filter(removeHMR), getExtensionGroup, getAssetOrFilename);
                    }
                }
                return [name, value];
            }));
        ...
        if (this.options.writeToDisk === 'auto') {
            if (this.inDevServer()) {
                ...
                if (this.compiler?.outputPath) {
                    return (0, node_path_1.relative)(this.compiler.outputPath, manifestPath).startsWith('..');
                }
            }
            return false;
        }
    ...
    handleNormalModuleLoader(compilation, loaderContext, module) {
        ...
        loaderContext.emitFile = (name, content, sourceMap, assetInfo) => {
            const info = Object.assign({
                rawRequest: module.rawRequest,
                sourceFilename: (0, node_path_1.relative)(compilation.compiler.context, module.userRequest),
            }, assetInfo);
            this.assetNames.set(contextRelativeKeys ? info.sourceFilename : (0, node_path_1.join)((0, node_path_1.dirname)(name), (0, node_path_1.basename)(module.userRequest)), name);
    ...
    inDevServer() {
        ...
        if ((serve === 'serve' && webpackPath && (0, node_path_1.basename)(webpackPath) === 'webpack') ||
            process.argv.some((arg) => arg.includes('webpack-dev-server'))) {
            return true;
        }
        return ((0, type_predicate_js_1.isObject)(this.compiler?.outputFileSystem) &&
            ('__vol' in this.compiler.outputFileSystem ||
                !Object.is(this.compiler.outputFileSystem, this.compiler.intermediateFileSystem)));
    }
    getOutputPath() {
        return (0, node_path_1.isAbsolute)(this.options.output)
            ? this.options.output
            : this.compiler
                ? (0, node_path_1.resolve)(this.compiler.outputPath, this.options.output)
                : '';
    }
...
exports.WebpackAssetsManifest = WebpackAssetsManifest;
